...
export function StockResearchTab({ stock, followedStocks, onSelectStock, onBack, onAskTeacher }: StockResearchTabProps) {
  ...
  const load = useCallback(async (refresh = false) => {
    ...
    try {
      ...
      const response = await fetch(`/api/stock-agents/cio-manager?symbol=${encodeURIComponent(symbol)}${suffix}`);
      const result = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(result?.detail || result?.error || '研究快照暂不可用');
      if (requestId === requestSequence.current) setPayload(result);
    } catch (loadError: any) {
