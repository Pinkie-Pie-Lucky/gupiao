...
export function StockResearchTab({ stock, followedStocks, onSelectStock, onFollowStock, onBack, onAskTeacher }: StockResearchTabProps) {
  ...
  const load = useCallback(async (refresh = false) => {
    ...
    try {
      ...
      const [response, industryResponse] = await Promise.all([
  ...
  const aiFailed = Boolean(payload?.agentMeta?.aiStatus === 'fallback' || payload?.opinion?.status === 'limited');
  ...
  return <div className="space-y-4 px-3 pb-24 pt-2">
    ...
    <StockResearchOverview stock={stock} stance={stance} researchStatus={manager.researchStatus} riskLevel={manager.riskLevel} conclusion={opinion.conclusion} evidenceCount={evidenceSet.size} loading={loading} error={error} aiFailed={aiFailed} />
