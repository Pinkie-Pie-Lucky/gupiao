...
2. **禁止列出具体指数数值** — 不得出现具体的指数点位、涨跌百分比等数字（这些已在市场概览中可视化展示）。要使用"三大指数集体下跌"、"大部分板块收涨"等概括性描述
...
输出严格 JSON 格式，不可夹带任何解释或评论：
{
  "summaryText": "一句话市场总结（50-80字），用概括性语言描述今日市场整体表现，禁止出现具体指数数值",
  "reasonBrief": "一段原因分析文本（100-150字），用通俗语言解释今天市场的三个核心驱动因素。每个因素用①②③编号单独成段，聚焦"为什么会这样""
}`;
