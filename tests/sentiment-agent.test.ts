...
function makePayload({ aiStatus = 'completed', status = 'limited', sourceAvailable = true } = {}) {
  ...
  return {
    ...
    sentimentSnapshot: { snapshotMeta: { sentimentVersion: 'stock-sentiment-v3' }, evidenceIds: [evidenceId], dataGaps: sourceAvailable ? ['缺少个股热度历史序列'] : ['舆情来源不可用'] },
    opinion: {
    ...
    agentMeta: { aiStatus, promptVersion: 'sentiment-v2', sentimentVersion: 'stock-sentiment-v3' },
  };
}

function validatePayload(payload: any) {
  assert.equal(payload.agentMeta?.sentimentVersion, 'stock-sentiment-v3');
